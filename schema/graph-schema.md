...
1. **连通性测试**：节点必须有至少一条边（`concept` / `targets` / `motivated_by` 任一非空；
   `concept_candidate` 算作候选边——否则新概念的第一个实例永远无法入图，方向 C 的概念发现无法启动）。
...
**① `system` —— 满足以下任一条：**
- 位于主数据流的**必经路径**上，**且没有注册/配置替换机制**（每个请求/消息都经过它，又换不掉它）
- 约束**某类操作的全部实例**（"所有工具调用"、"所有插件"、"所有模块加载"）
- 是**无法通过接口替换**的依赖（替换 = 重写一层）

注：必经路径上但**可通过注册机制替换**的能力单元（如可插拔的 channel 接口、
exclusive 注册槽位的引擎）按 ② 判定——可替换性把爆炸半径限制在了实现内部。
