...
> 最后更新：openclaw + hermes-agent（2026-06-19）
> 第二轮合并，种子库包含 44 条设计选择：12 条已升级为 Concept、11 条仅 openclaw—待观察、21 条仅 hermes-agent—待观察
...
**维度**：Extension Points
**问题陈述**：在 agent 生命周期中设计 hook 系统的拦截粒度时，如何决定事件的拆分程度——是少数粗粒度事件还是密集覆盖全生命周期？
**核心关切**：
...
**问题陈述**：在 token 计数不精确的前提下，如何决定上下文窗口溢出防护的压缩触发策略——是固定阈值还是多源保守选择？
**核心关切**：
...
**维度**：Performance Tradeoffs
**问题陈述**：在利用 LLM prompt caching 机制时，如何决定缓存边界的划分策略——是按消息类型切分还是按内容稳定性高低分离？
**核心关切**：
...
**Concept 状态**：仅 openclaw—待观察
> 反向检查：hermes 是单体仓库，所有依赖在同一个 pyproject.toml 中声明（通过 extras 分组而非独立包）。架构差异（monorepo vs monolith）决定了不同的依赖管理策略，属不同轴。
...
| hermes # | 条目 | 处理 |
|----------|------|------|
| 1 | 编排多轮 agent 对话的单一切入点 | 新条目：仅 hermes-agent—待观察 |
| 2 | 系统 prompt 内建自学习驱动 | 新条目：仅 hermes-agent—待观察 |
| 3 | 平衡命令执行的安全性与流畅性 | 合并至「工具执行安全门控」→ 已升级 [[concept/tool-security-gating]] |
| 4 | 发现和注册工具 (AST 扫描) | 新条目：仅 hermes-agent—待观察 |
...
| 7 | 组织工具为可组合的能力组 | 新条目：仅 hermes-agent—待观察 |
| 8 | 管理对话上下文超出窗口时的压缩策略 | 合并至「上下文窗口溢出防护」→ 已升级 [[concept/context-window-overflow-guard]]；同时参与「上下文压缩引擎可插拔架构」→ 已升级 [[concept/context-engine-pluggability]] |
| 9 | 集成外部工具服务（MCP 协议） | 新条目：仅 hermes-agent—待观察 |
| 10 | 管理 agent 生命周期事件的扩展点 | 合并至「生命周期 Hook 拦截粒度」→ 已升级 [[concept/lifecycle-hook-granularity]] |
| 11 | 管理可安装技能的互操作性 | 新条目：仅 hermes-agent—待观察 |
| 12 | 管理外部 skill 的安全信任 | 新条目：仅 hermes-agent—待观察 |
| 13 | 多轮对话中降低 token 成本 | 合并至「Prompt 缓存边界」→ 已升级 [[concept/prompt-cache-boundary]] |
| 14 | 分类管理工具的并行执行 | 新条目：仅 hermes-agent—待观察 |
| 15 | 为不同复杂度查询路由模型 | 新条目：仅 hermes-agent—待观察 |
| 16 | 管理 API 多凭证的速率限制 | 新条目：仅 hermes-agent—待观察 |
| 17 | 限制 agent 单次任务计算预算 | 新条目：仅 hermes-agent—待观察 |
| 18 | 管理可选依赖的优雅降级 | 合并至「可选依赖分层」→ 已升级 [[concept/optional-dependency-layering]] |
| 19 | 选择核心 API SDK 的架构锁定 | 新条目：仅 hermes-agent—待观察 |
| 20 | 实现依赖的可重复构建 | 合并至「依赖版本锁定」→ 已升级 [[concept/dependency-version-locking]] |
| 21 | 运行时选择多种后端实现 | 新条目：仅 hermes-agent—待观察 |
| 22 | 隔离测试环境以确保零残留 | 新条目：仅 hermes-agent—待观察 |
| 23 | 选择测试的抽象层级 | 新条目：仅 hermes-agent—待观察 |
| 24 | CI 中防范供给链攻击 | 新条目：仅 hermes-agent—待观察 |
| 25 | 为特定场景精简 agent 的工具面 | 新条目：仅 hermes-agent—待观察 |
| 26 | Gateway 模式下实现阻塞式审批 | 合并至「审批阻塞等待机制」→ 已升级 [[concept/approval-blocking-mechanism]] |
| 27 | 分离关注点防止单层膨胀 | 新条目：仅 hermes-agent—待观察 |
| 28 | 管理日志的安全性和可追溯性 | 新条目：仅 hermes-agent—待观察 |
| 29 | 管理后台进程的生命周期 | 新条目：仅 hermes-agent—待观察 |
...
| 32 | 缓存模型元数据加速启动 | 新条目：仅 hermes-agent—待观察 |
...
| 2 | 如何在消息驱动架构中创建主动触发路径 | 保持：仅 openclaw—待观察（反向检查：hermes 不面对此问题） |
...
| 7 | 如何在无代码情况下扩展 Agent 行为 | 保持：仅 openclaw—待观察（反向检查：hermes 无 markdown 零代码扩展机制） |
...
| 17 | 如何隔离 Channel SDK 故障域 | 保持：仅 openclaw—待观察（反向检查：hermes 单体仓库 vs openclaw monorepo） |
