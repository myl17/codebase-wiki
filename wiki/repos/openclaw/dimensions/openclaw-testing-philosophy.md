...
`test:startup:bench:check` 将 CLI 冷启动时间对比 `test/fixtures/cli-startup-bench.json` 中的基线 fixture，超出 budget 即 CI 失败。`test:perf:budget` 监控整体性能指标。**启动性能是一等公民，有专用 fixture 文件和 CI 检查。** 这是性能预算实践的典型形式。^[scripts/test-cli-startup-bench-budget.mjs:1-40]
