...
| 维度 | openclaw（Prompt 组装阶段批量注入） | hermes-agent（后台预取 + 不阻塞关键路径） |
|------|------------------------------------|----------------------------------------|
| **检索时机** | Prompt 组装阶段（`assemble`），同步检索 | 上一轮结束后入队后台线程，下一轮开始时消费缓存 |
| **注入位置** | System prompt section（与 identity/safety/skills 并列） | 用户消息尾部（`<memory-context>` 围栏，API-call 临时注入） |
| **确定性** | 高——同一轮内 system prompt 不变，记忆内容在整个 tool call 循环中稳定 | 中——依赖后台预取线程完成度，若未完成本轮记忆缺失 |
| **新增记忆可见性** | 不保证——组装发生在 API 调用前，当前轮工具写入的记忆要到下一轮组装才可见 | 不保证——预取上一轮结束前入队，当前轮写入要到下一轮预取才可见 |
| **用户感知延迟** | 有——同步检索延迟叠加到 prompt 组装耗时，影响 TTFT | 低——检索在后台完成，API 调用路径上只做缓存读取和 join（最多 3s） |
| **多后端处理** | promptBuilder 统一抽象——所有后端在组装阶段输出 string[]，后端差异对调用方透明 | 每个 provider 独立管理线程和缓存——provider 间隔离，互不阻塞 |
| **后端数量约束** | exclusive 槽位（1 个 promptBuilder，+ 多个 promptSupplement） | 内置必开启 + 最多 1 个外部 provider（增量式，内置不可移除） |
| **缓存/预取策略** | 无——每次组装重新检索（检索结果由向量引擎缓存决定） | 有——`queue_prefetch_all` 后台线程 + `_prefetch_result` 单槽位缓存 |
| **失败处理** | promptBuilder 异常由调用方处理，可能导致记忆 section 缺失 | 每个 provider 异常独立捕获（非致命），预取失败本轮的代价是记忆缺失 |
| **持久化边界** | 记忆内容写入 system prompt，理论上被持久化到会话文件 | 记忆注入暂时性（API-call time only），不污染持久化的 messages |
| **核心取舍** | 宁可每次组装都检索一遍也要保证同一轮内的确定性 | 宁可用上一轮结果（可能过时）也不阻塞当前轮 API 调用 |
...
| 需要记忆内容写入后立即可检索 | 两者均不满足 | 两者都不保证当前轮写入的记忆当前轮可见——这是时序设计的固有代价 |
