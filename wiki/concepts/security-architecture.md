...
来源：[[repos/codex-main/entities/execpolicy]]、[[repos/codex-main/entities/sandbox-abstraction]]、[[repos/codex-main/entities/shell-escalation]]
**解法**：PrefixRule 策略引擎 + SandboxManager 多平台沙箱 + EscalateServer 特权执行守护进程。
**实现**：
- `PrefixRule` 前缀模式匹配引擎对每个 shell 命令做出 Allow/Deny/Ask 三态决策 ^[codex-rs/execpolicy/src/rule.rs:56-67]
- `amend` 模块支持用户审批后动态追加 allow-prefix 规则到策略 ^[codex-rs/execpolicy/src/amend.rs:10-12]
- `SandboxManager` 通过平台自动选择（Landlock/Bubblewrap/Seatbelt/Windows）提供多层文件系统隔离 ^[codex-rs/sandboxing/src/manager.rs:14-21]
- `EscalateServer` 守护进程通过 Unix socket 接收特权命令，提供进程级隔离的权限提升 ^[codex-rs/shell-escalation/src/unix.rs:17-23]
- 策略规则和沙箱策略可从配置层加载，支持组织级的统一安全策略分发
**权衡**：前缀规则匹配简单高效，但缺乏语义分析（无法区分"rm 合法文件"和"rm 危险文件"）。沙箱的多平台覆盖最广，但不同后端的语义差异需 policy_transforms 做兼容。EscalateServer 的守护进程隔离是独特的安全设计，增加了架构复杂度。
...
| codex-main | 三层防线：PrefixRule 策略 + SandboxManager 沙箱 + EscalateServer 特权隔离 | 前缀匹配无语义分析，误报依赖规则粒度 | 通过配置层统一分发安全策略，支持组织级强制约束 |

## 演化记录
- 2026-06-25：初建，包含 nanobot, hermes-agent, openclaw
- 2026-06-28：新增 codex-main（PrefixRule 策略引擎 + SandboxManager 多平台沙箱 + EscalateServer 特权隔离）
