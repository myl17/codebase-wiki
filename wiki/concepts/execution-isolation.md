...
| codex-main | Environment 枚举 + ExecBackend trait + ExecutorFileSystem trait 三层抽象 | SandboxManager 多平台后端（Landlock/Bubblewrap/Seatbelt/Windows）；统一文件系统语义 | spawn-per-command + 本地/远程透明路由 + EscalateServer 特权执行守护进程 |
...
- 2026-06-28：新增 codex-main（Environment 三层抽象 + SandboxManager 多平台后端 + shell-escalation 特权执行）
