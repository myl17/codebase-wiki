...
Hermes Agent 的架构围绕七个核心抽象构建，形成清晰的[[分层架构]]：
...
| **ToolRegistry** | `tools/registry.py:100` | [[单例模式]]工具注册中心 — 收集工具 schema + handler，支持 MCP 动态刷新 |
...
5. → Agent 实例按 session 缓存以保持[[Prompt Caching]] ^[gateway/run.py:604-611]
...
- [[openclaw/dimensions/architecture]]
