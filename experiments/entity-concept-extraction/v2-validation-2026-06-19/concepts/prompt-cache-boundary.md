...
| nanobot | 工具定义排序（builtins 前缀 + MCP 后缀） | 缓存前缀稳定性（名称排序确保 builtins 始终在固定位置）；MCP 隔离（MCP 工具后缀分离，内置工具不受外部工具增删影响） | 内置工具数量膨胀会侵蚀缓存前缀空间 |
...
## nanobot — 工具定义排序（builtins 前缀 + MCP 后缀）

### 排序策略作为缓存边界

`agent/tools/registry.py:45-63` 的 `get_definitions()` 方法通过工具定义输出排序实现了一种与 system prompt 缓存边界互补的缓存稳定性策略。它不是通过插入缓存标记来切分边界，而是**通过固定输出顺序保证工具定义在每轮 API 调用中的字节级一致**——从而让上游的 system prompt 缓存边界能稳定命中工具定义所在的缓存段。

### 排序机制

```
builtins 工具（名称排序）→ MCP 工具（mcp_ 前缀，名称排序，后缀追加）
```

`get_definitions()` 将所有工具定义按名称排序后输出，其中 builtins（内置工具）排在前面、MCP 工具（`mcp_` 前缀）排在后面。名称排序消除了工具注册顺序的不确定性——无论工具以什么顺序注册到 registry，最终输出的定义列表字节级一致。

**MCP 隔离**：MCP 工具与内置工具在输出列表中明确分界——builtins 占固定前缀位置，MCP 工具作为后缀追加。这意味着外部 MCP 工具的增删只影响后缀部分——只要 MCP 工具集合不变，前缀缓存持续命中。即使有新的 MCP 工具加入或移除，builtins 前缀的缓存不受影响（因为 builtins 始终在前，且名称顺序固定）。

### 设计取向

| 满足的关切 | 接受妥协的关切 |
|-----------|--------------|
| 缓存前缀稳定性（名称排序确保 builtins 始终在固定位置；MCP 工具后缀分离，内置工具不受外部工具增删影响） | 内置工具数量膨胀会侵蚀缓存前缀空间（builtins 越多，每次 API 调用的缓存前缀 token 开销越大） |
| MCP 隔离（MCP 工具变动不影响 builtins 前缀的缓存命中） | |

### 代价

工具定义数量多时，缓存前缀本身占用大量 token 空间。与 system prompt 缓存边界互补——system prompt 缓存边界保证 prompt 前缀稳定，工具定义排序保证工具列表在每次调用中字节级一致。如果 builtins 数量持续增长（例如 agent 集成了越来越多的内置工具），缓存前缀的 token 开销会线性增长，即使命中缓存也需要支付这部分 token 的读取成本。

## 跨仓库对比

| 维度 | openclaw | hermes-agent | nanobot |
|------|----------|-------------|---------|
| **策略名称** | 稳定前缀 + 动态后缀分离 | `system_and_3` | 工具定义排序（builtins 前缀 + MCP 后缀） |
| **切入维度** | System prompt 内容切分 | 消息级别 block 分配 | 工具定义输出顺序 |
| **Breakpoint 数量** | 1 | 4（Anthropic 上限全用） | 依赖上游缓存边界（自身不管理 breakpoints） |
| **缓存覆盖** | 仅 system prompt 稳定前缀 | System prompt + 最近 3 条非 system 消息（滚动窗口） | 工具定义列表（builtins 前缀 + MCP 后缀），与 system prompt 缓存互补 |
| **边界标记方式** | `<!-- OPENCLAW_CACHE_BOUNDARY -->` 字符串插入 system prompt | `cache_control: { type: "ephemeral" }` 逐条注入 message 对象（deep copy 后操作） | 无标记——通过名称排序保证输出顺序的字节级确定性 |
| **TTL 策略** | Endpoint-aware：`api.anthropic.com` / `aiplatform.googleapis.com` → 1h，其他 → short；可通过 `cacheRetention` config 或 `PI_CACHE_RETENTION` 环境变量覆盖 | 默认 5min，可通过 `ANTHROPIC_CACHE_TTL` 切换到 1h | 无独立 TTL——依赖上游缓存边界的 TTL 策略 |
| **多平台兼容路径** | Cache 标记字符串平台无关，per-provider payload policy 各自解析并应用平台缓存协议 | 仅 Anthropic/OpenRouter 路径自动启用——其他 provider 不应用缓存，通过检查 provider 类型分支 (`run_agent.py:809-812`) | 排序策略本身平台无关——任何 provider 的工具定义输出都受益于确定性排序 |
| **Agent 实例复用** | 未从文档中检出显式的实例缓存机制 | `GatewayRunner` 缓存 `AIAgent` 实例 (`gateway/run.py:604-611`) | 不涉及实例复用——排序在工具注册阶段完成，与 agent 生命周期无关 |
| **每次 turn 开销** | 一次性边界注入（system prompt 构建阶段），后续 turn 无额外操作 | 每次 turn deep copy messages + 逐条注入 markers | O(n log n) 排序开销（工具定义数 n），每次 `get_definitions()` 调用时执行 |
| **缓存命中确定性** | 高——稳定前缀内容完全不变，缓存一定命中直到 TTL 过期 | 中——breakpoint 1 确定命中，breakpoints 2-4 命中取决于消息是否被滚动覆盖 | 取决于上游缓存边界——排序保证工具定义字节级一致，但不直接管理缓存标记 |
| **外部工具影响** | 不受影响——工具 schema 在稳定前缀中，新增工具会改变前缀内容导致缓存重写 | 不受影响——工具 schema 在 system prompt（breakpoint 1）中 | MCP 工具增删仅影响后缀，builtins 前缀缓存不受影响 |
| **核心取舍** | 宁可少用 breakpoints 也要保证覆盖内容的绝对稳定（稳定性 > 配额利用率） | 宁可用满 breakpoints 也要最大化缓存保护面（覆盖率 > 稳定性） | 宁可固化输出顺序也要保证工具定义的字节级确定性（顺序确定性 > 灵活性） |
...
| 内置工具数量多且 MCP 工具频繁变动 | nanobot 的工具定义排序 | MCP 工具增删仅影响后缀，builtins 前缀缓存不受影响；名称排序消除注册顺序不确定性 |
| System prompt 缓存边界已建立，需工具定义侧配合保证前缀稳定 | nanobot 的工具定义排序 + openclaw 的稳定前缀分离 | 排序保证工具定义在 system prompt 中字节级一致，与缓存边界互补 |
| 多 agent 实例共享同一工具注册表 | nanobot 的工具定义排序 | 排序在 registry 层完成，所有 agent 实例输出的工具定义顺序一致，缓存可跨实例共享 |
...
| nanobot | `agent/tools/registry.py` | 45-63 | `get_definitions()` 按名称排序 builtins 在前、MCP 工具（`mcp_` 前缀）在后，保证输出顺序字节级确定性 |
