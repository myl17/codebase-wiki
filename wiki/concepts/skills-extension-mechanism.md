...
来源：[[repos/codex-main/entities/skills-system]]
**解法**：`CODEX_HOME/skills/` 目录声明式技能文件 + 编译时嵌入式系统技能 + 指纹校验避免重复安装。
**实现**：
- 技能以 Markdown 文件形式存放在 `CODEX_HOME/skills/` 目录，文件内容描述触发条件、行为和示例 ^[codex-rs/skills/src/lib.rs:32]
- 系统技能通过 `include_dir!` 宏在编译时嵌入 `src/assets/samples`，运行时安装到 `skills/.system` ^[codex-rs/skills/src/lib.rs:10-11]
- Marker 文件指纹校验：哈希匹配时跳过安装，哈希变化或不存在时清除旧目录重新安装 ^[codex-rs/skills/src/lib.rs:39-50]
- 元数据预算管理：`default_skill_metadata_budget` 控制技能元数据在系统提示词中的 token 配额 ^[codex-rs/core/src/session/mod.rs:33]
- 技能加载由 `skills_load_input_from_config` 从配置层解析技能根目录和过滤规则 ^[codex-rs/core/src/session/mod.rs:34]
- `core-skills` crate 定义内置核心技能（文件操作、Web 搜索、代码执行等）^[codex-rs/core-skills/src/lib.rs]
**权衡**：声明式 Markdown 格式门槛低——用户无需编程即可创建技能。系统技能的编译时嵌入+指纹校验避免了每次启动的 IO 开销。但来源仅限于本地（无社区市场/远程安装），安全策略依赖文件系统权限而非框架级隔离。
...
| codex-main | 两层本地（CODEX_HOME/skills/ + 编译嵌入 system skills） | 系统技能自动安装（指纹校验）；用户技能手动放置 | 元数据预算控制 + 配置驱动的过滤规则 |

## 演化记录
- 2026-06-25：初建，包含 nanobot, hermes-agent, openclaw
- 2026-06-28：新增 deepagents（Agent Skills 规范 + 纯渐进披露 + 多源合并）
- 2026-06-28：新增 codex-main（声明式 Markdown 技能 + 编译嵌入系统技能 + 指纹校验安装）
